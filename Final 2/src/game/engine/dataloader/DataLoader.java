	...
	public static HashMap<Integer, WeaponRegistry> readWeaponRegistry() throws IOException
{
    ...
    while (br.ready())
    {
        ...
        if (data.length != 7 && data.length != 5) // Adjusted for new column
        {
            throw new InvalidCSVFormat(nextLine);
        }

        if (data.length == 7) // Adjusted for new column
        {
            reg = new WeaponRegistry(Integer.parseInt(data[0]), Integer.parseInt(data[1]),
                    Integer.parseInt(data[2]), data[3], Integer.parseInt(data[4]), Integer.parseInt(data[5]), data[6]); // Added new column
        } else
        {
            reg = new WeaponRegistry(Integer.parseInt(data[0]), Integer.parseInt(data[1]),
                    Integer.parseInt(data[2]), data[3], data[4]); // Added new column
        }
